...
void plot_trigEff_3_years() {
	...
	title = "Electron #eta, p_{T} > 40 [GeV]";
	}
	...
	TString infile = "results" + postfix + "/2016_" + dataset + ".root";
	//TString infile = "results_loose_baseline/2016_SingleElectron.root";
	...
	for (int i = 0; i < h_met_TEff->GetPaintedGraph()->GetN(); i++)
	{
        ...
        leg->AddEntry(h_met_TEff,"2016 " + dataset,"lep");
        //leg->AddEntry(h_met_TEff,"Stop 0l","lep");
	...
	//TH1F* h_met_denom2 = (TH1F*) f_in2->Get("TrigAnalyzerMiniAOD/" + refTrg + "_RA2b_veto");
	//TH1F* h_met_num2 = (TH1F*) f_in2->Get("TrigAnalyzerMiniAOD/" + sigTrg + "_RA2b_veto");
	...
	//TH1F* h_met_denom3 = (TH1F*) f_in2->Get("TrigAnalyzerMiniAOD/" + refTrg + "_RA2b_jet");
	//TH1F* h_met_num3 = (TH1F*) f_in2->Get("TrigAnalyzerMiniAOD/" + sigTrg + "_RA2b_jet");
	...
	for (int i = 0; i < h_met_TEff3->GetPaintedGraph()->GetN(); i++)
	{
        ...
        leg->AddEntry(h_met_TEff3,"2018 " + dataset,"lep");
        //leg->AddEntry(h_met_TEff3,"+ RA2b veto & jet","lep");


        h_met_MG->Draw("ape");

        h_met_MG->GetXaxis()->SetTitle(title);
        h_met_MG->GetXaxis()->SetLimits(h_met_denom->GetXaxis()->GetXmin(), h_met_denom->GetXaxis()->GetXmax());
	...
	if(year == "2018") lumi = 59;
        ...
        TString lumi_and_energy = "#bf{Run II data (13TeV)}";
        latex.DrawLatex(0.6,0.91,lumi_and_energy);
/*
	myCanvas->cd();
